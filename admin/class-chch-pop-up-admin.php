...
class CcPopUpAdmin {
	...
	public function chch_pu_register_post_type() {
		...
		$labels = array(
			'name' => _x( 'Pop-Up Chop Chop', 'Post Type General Name', $domain ),
			...
			'all_items' => __( 'Pop-Ups Chop Chop', $domain ),
	...
	function chch_pu_initialize_cmb_meta_boxes() {
		...
		$gdpr_metabox = new_cmb2_box( array(
			'id' => 'chch-puf-metabox-gdpr',
			'title' => __( 'GDPR', $domain ),
			'object_types' => array( 'chch-pop-up' ),
		) );

		$gdpr_metabox->add_field( array(
			'name' => __( 'Enable GDPR?', $domain ),
			'desc' => __( 'Enable or disable the GDPR checkbox.', $domain ),
			'id' => $prefix . 'gdpr_status',
			'type' => 'radio_inline',
			'default' => 'no',
			'options' => array(
				'yes' => __( 'Enable', $domain ),
				'no' => __( 'Disable', $domain ),
			),
		) );

		$gdpr_metabox->add_field( array(
				'name' => __( 'Label', $domain ),
				'id' => $prefix . 'gdpr_label',
				'type' => 'text',
				'default' => 'I agree to have my personal data processed.' )
		);
