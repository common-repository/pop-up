	...
	<table class="form-table cmb_metabox">
		<tbody>
			<tr class="cmb-type-radio_inline cmb_id__chch_pop_up_time_newsletter cmb-inline">
				<th class="c1"><label for="_chch_pop_up_time_newsletter">Newsletter Status:</label></th>

				<td>
					<?php

$newsletter = get_post_meta( $post->ID, '_chch_pop_up_newsletter', true ) ? get_post_meta( $post->ID, '_chch_pop_up_newsletter', true ) : 'yes';

?>

					<ul class="cmb_radio_list cmb_list">
						<li><input class="cmb_option" name="_chch_pu_newsletter" id="_chch_pop_up_time_newsletter1" value="yes" type="radio" <?php

if ( $newsletter == 'yes' )
  echo 'checked';

?> /> <label for="_chch_pop_up_time_newsletter1">Active</label></li>

						<li><input class="cmb_option" name="_chch_pu_newsletter" id="_chch_pop_up_time_newsletter2" value="no" type="radio" <?php

if ( $newsletter == 'no' )
  echo 'checked';

?> /> <label for="_chch_pop_up_time_newsletter2">Inactive</label></li>
					</ul>
					<span class="cmb_metabox_description">Show or hide the newsletter subscribe form.</span>
				</td>
			</tr>

			<tr class="cmb-type-select cmb_id__chch_pop_up_time_newsletter_adapter">
				<th class="c1"><label for="_chch_pu_newsletter_adapter">Save E-mails to:</label></th>

				<td>
					<select id="_chch_pop_up_save_emails" name="_chch_pop_up_save_emails" class="cmb_select">
					...
					<!-- <a href="http://ch-ch.org/pupro" target="_blank">Get Pro</a> -->
				</td>
			</tr><?php
...
?>" />
<br/>
<span class="cmb_metabox_description">"Placeholder" field is mandatory for the form to work correctly.</span>
				</td>
			</tr>
		</tbody>
	</table>
	<input type="hidden" name="chch-pu-newsletter-nonce" value="<?php

	echo wp_create_nonce( 'chch-pu-newsletter-nonce' );
